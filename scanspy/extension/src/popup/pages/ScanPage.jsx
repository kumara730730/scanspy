    ...
    <PublicShell className="py-8">
      ...
      <SectionWrap>
        <RevealInView>
          ...
          {data?.status === 'failed' && data?.errors?.length > 0 && (
            <div className="mt-4 rounded-lg border border-red-500/50 bg-red-500/10 p-4">
              <h3 className="mb-1 text-sm font-semibold text-red-200">Scan Failed</h3>
              <p className="text-xs text-red-300">{data.errors[data.errors.length - 1]}</p>
              <p className="mt-2 text-[10px] text-red-400/70 italic">Tip: Check if the target URL is reachable and allows requests from extensions.</p>
            </div>
          )}
        </RevealInView>
      </SectionWrap>
